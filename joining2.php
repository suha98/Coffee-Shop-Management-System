...
<th scope="col" style="width: 70px;">Sale ID</th>
      ...
      <th scope="col" style="width: 100px"> Member Name</th>
...
$sql = "SELECT * FROM sales INNER JOIN members ON sales.ordered_by = members.member_id";
$result = mysqli_query($conn, $sql);
  while ($row = mysqli_fetch_assoc($result)) {
    echo "<tr><td>". $row["sale_id"]. "</td><td>". $row["sale_date"]. "</td><td>". $row["ordered_by"] . "</td><td>". $row["received_by"] . "</td><td>". $row["sale_item"]. "</td><td>".$row["sale_price"]. "</td><td>" . $row["sale_quantity"]. "</td><td>" . $row["sale_total"]. "</td><td>". $row["member_name"]. "</td><td>". $row["member_address"] ."</td><td>". $row["member_date"]. "</td><td>".$row["member_contact"]. "</td><td>" . $row["member_email"] ."</td></tr>";
  }
